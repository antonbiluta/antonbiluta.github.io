...
/**
 * Credits http://denilson.sa.nom.br/html5-knob/
 */
if (!window.XKnob) {
	(function() {
			...
			prototype: Object.create(HTMLElement.prototype, {
				...
				attributeChangedCallback: {
					value: function(attrName, oldVal, newVal) {
						...
						if (
							...
							["disabled", "readonly"].indexOf(attrName) > -1
						) {
							if (newVal === null) {
				...
				_update_innerHTML: {
					value: function() {
						...
						if (this._svgsymbolid) {
							symbol = this.ownerDocument.getElementById(
								this._svgsymbolid
							);
							if (
								symbol &&
								symbol.tagName.toLowerCase() === "symbol"
							) {
						...
						this.shadowRoot.innerHTML =
							...
							'  <use class="knob_gfx" xlink:href="#' +
							id +
							'" x="-1" y="-1" width="2" height="2" style="cursor: default; pointer-events: auto; -webkit-touch-callout: none; -ms-user-select: none; -moz-user-select: none; -webkit-user-select: none; user-select: none;"/>' +
							"  </g>" +
							"</svg>";
				...
				_update_value: {
					value: function() {
						...
						if (
							Number.isFinite(this._divisions) &&
							this._divisions >= 2
						) {
							this._value =
								Math.round(this._value * this._divisions) /
								this._divisions;
						}

						// Clamping to the defined min..max range.
						if (
							Number.isFinite(this._max) &&
							this._value > this._max
						) {
							this._value = this._max;
						}
						if (
							Number.isFinite(this._min) &&
				...
				_update_gfx_rotation: {
					value: function() {
						if (this.shadowRoot) {
							var elem = this.shadowRoot.querySelector(
								".knob_gfx"
							);
							if (elem) {
								elem.style.transform =
									"rotate(" + this._value * 360 + "deg)";
							}
						}
					}
				},
