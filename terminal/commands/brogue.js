import { getScreen, showTemplateScreen, addTemplate } from "../util/screens.js";
...
class Game {
	...
	constructor(el, settings = {}) {
		...
		this.output = el.appendChild(output);
		addTemplate("layout", el);
		...
		document.querySelector(".layout .mid").appendChild(canvas);
		this.updateControls();
	}

	async message(text) {
		this.output.innerHTML = "";
		await type(text, { initialWait: 0 }, this.output);
		await pause(2);
		this.output.innerHTML = "";
	}
...
async function brogue() {
	...
	return new Promise(async resolve => {
		...
		new Game(gameScreen, settings);
	});
}

const template = "brogue";
